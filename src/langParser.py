...
class Parser:
    def __init__(self, builtins={k: v[0] for k,v in BUILTIN_FUNCTIONS.items()}):
        self.tokens = []
        self.ast = []
        self.current = 0
        self.function_returns = builtins
        self.variable_types = {}

    def throwError(self, message):
        std.error(message + f" | Line: {self.curToken().line} Col: {self.curToken().column}")
    ...
    def parseVariableAssignment(self):
        ...
        if variable not in self.variable_types:
            self.throwError(f"Variable {variable} has not been declared")
    ...
    def parseDataType(self):
        ...
        sub_type = None
        if data_type.type in (TOKENTYPE.STRUCT) :
